...
def convert_tokens_to_bm25s_by_full_word_for_msmarco_doc(full_word_idfs, tokens, words, default_idf, is_doc):
    import collections
    ...
    while token_idx < len(tokens) and word_idx < len(words):
        ...
        while tokens[token_idx] == "[SEP]" and words[word_idx] != "[SEP]":
            word_idx += 1
        word = words[word_idx]
        # Attention! Token is uncased!
        if temp_token_agg == word or temp_token_agg.lower() == word or (token_idx < len(tokens) - 1 and tokens[token_idx + 1] == "[SEP]"):
            idf = full_word_idfs[word] if word in full_word_idfs else default_idf
            for i in range(accumulation):
                tf = tfs[word] / n_words
                if is_doc:
                    atf = tf * (FLAGS.BM25_K1 + 1) / (tf + FLAGS.BM25_K1 * (1 - FLAGS.BM25_B + FLAGS.BM25_B * n_words / FLAGS.BM25_AVGDL_D))
                else:
                    atf = tf * (FLAGS.BM25_K1 + 1) / (tf + FLAGS.BM25_K1 * (1 - FLAGS.BM25_B + FLAGS.BM25_B * n_words / FLAGS.BM25_AVGDL_Q))
                bm25s_list.append(idf*atf)
            temp_token_agg = ""
            accumulation = 1
            word_idx += 1
            token_idx += 1
...
class FullTokenizerFullWordIDF(object):
    ...
    def __init__(self, vocab_file,full_word_idf_file, do_lower_case=True):
    ...
    def convert_tokens_to_bm25s_by_full_word_for_msmarco_doc(self, tokens, words, default_idf, is_doc):
        return convert_tokens_to_bm25s_by_full_word_for_msmarco_doc(self.full_word_idfs, tokens, words, default_idf, is_doc)    

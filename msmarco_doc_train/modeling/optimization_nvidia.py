...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, hvd=None, manual_fp16=False, use_fp16=False, num_accumulation_steps=1,
                     optimizer_type="adam", allreduce_post_accumulation=False, freeze_embedding=False):
    ...
    adjusted_init_lr = init_lr * \
        (init_lr / decayed_learning_rate_at_crossover_point)
    ...
    learning_rate = tf.constant(
        value=adjusted_init_lr, shape=[], dtype=tf.float32)
    ...
    if hvd is not None and (num_accumulation_steps == 1 or (not allreduce_post_accumulation)):
        optimizer = hvd.DistributedOptimizer(
            optimizer, sparse_as_dense=True, compression=Compression.fp16 if use_fp16 or manual_fp16 else Compression.none)
    ...
    if freeze_embedding:
        tvars = [v for v in tvars if 'word_embeddings' not in v.name]
    ...
    if num_accumulation_steps > 1:
        ...
        reset_step = tf.cast(tf.math.equal(local_step %
                                           num_accumulation_steps, 0), dtype=tf.bool)
        local_step = tf.cond(reset_step, lambda: local_step.assign(
            tf.ones_like(local_step)), lambda: local_step.assign_add(1))

        grads_and_vars_and_accums = [(gv[0], gv[1], accum_vars[i]) for i, gv in enumerate(
            grads_and_vars) if gv[0] is not None]
        grads, tvars, accum_vars = list(zip(*grads_and_vars_and_accums))

        all_are_finite = tf.reduce_all([tf.reduce_all(tf.is_finite(
            g)) for g in grads]) if manual_fp16 or use_fp16 else tf.constant(True, dtype=tf.bool)
        batch_finite = tf.cond(reset_step,
                               lambda: batch_finite.assign(tf.math.logical_and(
        ...
        (clipped_grads, l2_norm) = tf.clip_by_global_norm(
            grads, clip_norm=1.0,
            use_norm=tf.cond(
                all_are_finite,
        ...
        accum_vars = tf.cond(reset_step,
                             lambda: [accum_vars[i].assign(
                                 grad) for i, grad in enumerate(clipped_grads)],
        ...
        update_step = tf.identity(tf.cast(tf.math.equal(
            local_step % num_accumulation_steps, 0), dtype=tf.bool), name="update_step")
        update_op = tf.cond(update_step,
                            lambda: update(accum_vars), lambda: tf.no_op())

        new_global_step = tf.cond(tf.math.logical_and(update_step, tf.cast(hvd.allreduce(
            tf.cast(batch_finite, tf.int32)), tf.bool)), lambda: global_step+1, lambda: global_step)
        ...
        new_global_step = tf.cond(
            all_are_finite, lambda: global_step + 1, lambda: global_step + 1)### In order to align with fidelity pipeline, prevent steps not same for evaluation and train
        new_global_step = tf.identity(new_global_step, name='step_update')
        train_op = tf.group(train_op, [global_step.assign(new_global_step)])
    return train_op, learning_rate, l2_norm
...
class LAMBOptimizer(tf.train.Optimizer):
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None,
        ...
        for (grad, param) in grads_and_vars:
            ...
            update = next_m_unbiased / \
                (tf.sqrt(next_v_unbiased) + self.epsilon)
